...
- [x] **2.1 SPX alpha benchmark** — `/performance` shows per-signal return vs SPX over the same window → net alpha (the CV headline "outperformed SPX by X%")
  - [x] SPX daily closes stored in `price_history` as `^GSPC` — `refreshBenchmark(store)` in web/src/research.js called once per analyze/scan/watch/spy (CLI + bot); `npm run analyze -- spy`-style backfill via `node src/cli.js spy`
...
- [x] **Strategy pool architecture** — `web/src/strategies/` registry (`getStrategy`/`listStrategies`); Trend Follower is the ONLY active strategy (id `trend`). `analyze()` takes `strategy` param, uses strategy weights + guards. `/strategies` command lists the pool.
...
## Guiding principles
- Engine (`web/src/analysis.js`, `market.js`, `comps.js`, `research.js`, `strategies/`) is dependency-free (except `market.js`→yahoo-finance2) and lives in `web/src/` — the WEB APP is the product, the Discord bot (root `src/`) consumes it via `../web/src/…` imports. Single canonical copy.
...
- Vercel build: root directory = `web/`, GitHub integration (clones full repo so `../web/src/…` engine imports work). Vercel installs deps ONLY in the app dir → the engine MUST live under `web/` or Turbopack can't resolve `yahoo-finance2` for files outside it. Do NOT move engine files back to repo root.
